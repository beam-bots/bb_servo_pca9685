...
if Code.ensure_loaded?(Igniter) do
  defmodule Mix.Tasks.BbServoPca9685.Upgrade do
    @shortdoc "Lifts `reverse?` into joint transmissions"
    @moduledoc """
    #{@shortdoc}

    `BB.Servo.PCA9685.Actuator` no longer takes a `reverse?` option. Polarity
    now lives in the joint-level `transmission` block in BB.

    This upgrader rewrites every robot module in the project: for each
    actuator with `BB.Servo.PCA9685.Actuator` as its driver, it removes
    ...
    def igniter(igniter) do
      Transmission.lift_reverse_question(igniter, BB.Servo.PCA9685.Actuator)
    end
  end
end
